...
class DistillResidualRLImgModel(DistillResidualRLModel):
    ...
    def __init__(self, 
        ...
        print(f'DistillResidualRLImgModel initialized with parameters inherited from DistillResidualRLModel')
    ...
    def _load_pretrained_policy(self, checkpoint_path: str, device: str):
        ...
        Returns:
            Loaded and frozen pretrained policy
        """
    ...
    def get_action(self, state: torch.Tensor, noise: torch.Tensor, return_pretrained_actions: bool = False):
        ...
        with torch.no_grad():
            # State already contains merged visual features, flatten for forward_from_features
            B = state.shape[0]
            state_flat = state.view(B, -1)
            
            pretrained_sample = self.pretrained_flow_policy.forward_from_features(
                features=state_flat,
                init_noise=noise
            )
            pretrained_actions = pretrained_sample.trajectories.detach()  # (B, horizon_steps, action_dim)
    ...
    def actor_loss(
        ...
        """
        Actor loss for residual RL with multi-z sampling 
        ...
        2. After warmup: BC filtering when conditions are met
        """
        B = state.shape[0]
        ...
        if in_warmup:
            # WARMUP PHASE: Simple loss without filtering 
            ...
            # POST-WARMUP PHASE: With BC filtering
