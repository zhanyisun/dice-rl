...
class VPGDiffusion(DiffusionModel):
    ...
    def p_mean_var_from_features(
        ...
        """
        Override to handle actor vs actor_ft for fine-tuning with features.
        
        Args:
            x: (B, Ta, Da) - current noisy actions
            t: (B,) - timesteps  
            features: (B, cond_steps, obs_dim) - pre-extracted augmented features
            index: (B,) - index in denoising sequence (for DDIM)
            use_base_policy: whether to use frozen pretrained policy
            deterministic: whether to use deterministic sampling
        """
        # Get noise prediction using pre-extracted features
        noise = self.actor.forward_from_features(x=x, time=t, state=features)
        ...
        # overwrite noise for fine-tuning steps
        if len(ft_indices) > 0:
            noise_ft = actor.forward_from_features(
                x=x[ft_indices], 
                time=t[ft_indices], 
                state=features[ft_indices]
            )
            noise[ft_indices] = noise_ft

        # The rest is identical to p_mean_var
        # Predict x_0
        if self.predict_epsilon:
            if self.use_ddim:
                """
        ...
        if self.use_ddim:
            ...
            if deterministic:
                ...
                # For features, pass as state in dict for eta network
                etas = self.eta({"state": features}).unsqueeze(1)  # B x 1 x (Da or 1)
            sigma = (
                etas
                * ((1 - alpha_prev) / (1 - alpha) * (1 - alpha / alpha_prev)) ** 0.5
            ).clamp_(min=1e-10)
    ...
    def forward_from_features(
        ...
        Override to use p_mean_var_from_features with actor/actor_ft handling.
        
        Args:
            features: (B, cond_steps, obs_dim) - pre-extracted augmented features
            deterministic: whether to use deterministic sampling
            return_chain: whether to return the chain of denoised actions
            return_noise: whether to return the noise used
            use_base_policy: whether to use frozen pretrained policy
            
        Returns:
            Sample: namedtuple with trajectories and chain
            noises (if return_noise=True)
        """
        ...
        for i, t in enumerate(t_all):
            ...
            mean, logvar, _ = self.p_mean_var_from_features(
                x=x,
                t=t_b,
                features=features,
                index=index_b,
        ...
        noises_tensor = None
        if return_chain:
            chain_tensor = torch.stack(chain, dim=1)        # (B, K_f+1, Ta, Da)
        if return_noise:
            noises_tensor = torch.stack(noises, dim=1)      # (B, K_f,   Ta, Da)
        sample = Sample(x, chain_tensor)
        return (sample, noises_tensor) if return_noise else sample
    ...
    def forward(
        ...
        if self.use_ddim:
            ...
            t_all = list(reversed(range(self.denoising_steps)))
        chain = [] if return_chain else None
        noises = [] if return_noise else None          # to hold z_k
        if return_chain:
            if not self.use_ddim and self.ft_denoising_steps == self.denoising_steps:
                chain.append(x)
            if self.use_ddim and self.ft_denoising_steps == self.ddim_steps:
    ...
    def replay_forward_w_noise(
            self,
            *,                       # keyword-only for clarity
            cond:     dict,          # observations,   B × …
            deterministic=False,
            latent_K: torch.Tensor,  # initial latent x^{K},  (B, T_a, D_a)
            noises:   torch.Tensor   # stored z_k,      (B, K, T_a, D_a)
        ) -> torch.Tensor:
        """
        Re-run *all* reverse steps with the exact Gaussian noises that were
        saved during data collection (fine-tuning all denoising steps).

        Parameters
        ----------
        cond      : observation dict (same structure as in `forward`)
        latent_K  : initial latent x^{K}  (the `w` that was sampled at t=K)
                    Typically   latent_K = chains_b[:, 0]   from the buffer.
        noises    : Gaussian noises in the **same order** they were stored
                    during rollout:
                    noises[:, 0] == z_{K-1},
                    noises[:, 1] == z_{K-2},
                    …,
        ...
        Returns
        -------
        a0  : (B, T_a, D_a)   – differentiable w.r.t. θ
        """
        device = self.device
        latent_K = latent_K.to(device).requires_grad_(True)   # start point
        noises   = noises.to(device)
        min_sampling_denoising_std = self.get_min_sampling_denoising_std()

        B, K, Ta, Da = noises.shape
        x = latent_K                                        # x^{K}

        # ------------------------------------------------------------------
        # Build the list of timesteps (descending) that were fine-tuned.
        # For "all steps" this is the full reverse schedule.
        # ------------------------------------------------------------------
        if self.use_ddim:
            # self.ddim_t is already descending (K-1 .. 0)
            t_all = self.ddim_t
        else:
            # reversed(range(K))  gives  K-1, … , 1, 0
            t_all = list(reversed(range(self.denoising_steps)))

        assert len(t_all) == K, "noises tensor must contain all K steps"

        # ------------------------------------------------------------------
        # Iterate through the reverse steps, consuming the stored z_k
        # in **the same order** they were generated.
        # ------------------------------------------------------------------
        for i, t in enumerate(t_all):               # i = 0 … K-1
            t_b     = make_timesteps(B, t, device)
            index_b = make_timesteps(B, i, device)  # positional idx for DDIM

            mu, logvar, _ = self.p_mean_var(
                x,          # current latent
                t_b,
                cond,
                index=index_b,
                deterministic=deterministic        # keep stochasticity
            )
            std = torch.exp(0.5 * logvar)
            # Determine noise level
            if self.use_ddim:
            ...
            x   = mu + std * noises[:, i]   # use z_{t} exactly as stored
            # clamp action at final step
            if self.final_action_clip_value is not None and i == len(t_all) - 1:
                x = torch.clamp(
                    x, -self.final_action_clip_value, self.final_action_clip_value
    ...
    def get_logprobs_from_features(
        self,
        cond,
        chains,
        get_ent: bool = False,
        ...
        # Forward pass with previous chains
        next_mean, logvar, eta = self.p_mean_var_from_features(
            chains_prev,
            t_all,
            features=cond['state'],
            index=indices,
