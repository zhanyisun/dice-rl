...
class PPODiffusion(VPGDiffusion):
    ...
    def loss_from_features(
        ...
        """
        PPO loss using pre-extracted features
        Same as loss() but uses get_logprobs_from_features and forward_from_features
        
        obs: dict with key state containing augmented features
            state: (B, To, feature_dim) - pre-extracted visual features + state
        chains: (B, K+1, Ta, Da)
        returns: (B, )
        values: (B, )
        advantages: (B,)
        ...
        """
        # Get new logprobs for denoising steps from T-1 to 0 - entropy is fixed fod diffusion
        newlogprobs, eta = self.get_logprobs_from_features(
            obs,
            chains,
            get_ent=True,
        )
        ...
        if use_bc_loss:
            ...
            # Get counterfactual teacher actions
            samples = self.forward_from_features(
                features=obs["state"].float().to(self.device),  # B x obs_dim (augmented)
                deterministic=False,
                return_chain=True,
                use_base_policy=True,
            )
            # Get logprobs of teacher actions under this policy
            bc_logprobs = self.get_logprobs_from_features(
                obs,
                samples.chains,  # n_env x denoising x horizon x act
                get_ent=False,
                use_base_policy=False,
