"""Pre-training flow matching policy with image observations."""
...
from agent.pretrain.train_flow_matching_agent import TrainFlowMatchingAgent
from agent.pretrain.train_agent import batch_to_device


class TrainFlowMatchingImgAgent(TrainFlowMatchingAgent):
    """
    Flow matching pretraining agent for image observations.
    
    This extends TrainFlowMatchingAgent to properly handle both state and RGB observations
    ...
    def __init__(self, cfg):
        ...
        # If using 6D rotation and absolute actions, validate configuration
        if use_6d_rot and abs_action:
            ...
            # Check that paths contain 'abs' to indicate absolute action configuration
            paths_to_check = [
        ...
        log.info(f"TrainFlowMatchingImgAgent initialized with obs_dims: {self.obs_dims.keys()}")
        self.val_freq = 100
